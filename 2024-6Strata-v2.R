...
# ObsModel=c(10,2) #Tweedie Model - DOESN'T WORK
# RhoConfig=c("Beta1"=0,"Beta2"=0,"Epsilon1"=0,"Epsilon2"=0)
...
# Remove Unsatisfactory tows: EVENT
unique(dat.event$TrawlPerformance)

dat.event.3 <- dat.event.2 %>% dplyr::filter(TrawlPerformance %in% c("G/S","S","G"))
...
# Join data together
dat.input <- dat.event.3 %>% left_join(dat.catch.2, by=c("SampleYear"="SampleYear",
                                                         "StationID"="StationID"))
...
# Six strata
strata.limits <- data.frame(
  'STRATA' = c("All areas","Bering_Strait","NBS","NBS_NW","SBS_NE","SBS_SW","SBS_SE"),
